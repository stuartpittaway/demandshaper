...
function teslavehicle_calc_modes(reset_timeleft) {
...
function schedule_info_teslavehicle(forecast_units,mean,peak) {
...
//console.log(inputs)
    ...
    if (inputs.battery_level!=undefined) {
        ...
        schedule.settings.current_soc = inputs.battery_level.value*0.01;
        teslavehicle_calc_modes(false);
        if (schedule.settings.current_soc!=last_soc) on_UI_change();
    }

    if (inputs.charge_limit_soc!=undefined) {
        var target_soc=inputs.charge_limit_soc.value*0.01;
        if (schedule.settings.target_soc!=target_soc) {
            schedule.settings.target_soc=target_soc;
            on_UI_change();
        }
    }
...
function teslavehicle_fetch_ovms_soc(callback) { }
...
//        schedule.settings.target_soc = battery.charge_limit_soc*0.1;
        schedule.settings.target_soc = 1;
        teslavehicle_calc_modes(true);
        on_UI_change();
    });
